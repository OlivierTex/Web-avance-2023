...
const VideoPage = () => {
    ...
    try {
      ...
      if (isLiked) {
        ...
        if (existingFavorites.length === 0) {
          ...
          console.log(
            "Cette combinaison user_session.id et IDivideo existe déjà dans la table favoris.",
          );
        }
      }
    } catch (error) {
  ...
  return (
    <div>
      ...
      <video
        className="image-preview w-auto h-auto object-cover mx-auto max-h-screen m-4 cursor-pointer border-4 border-custom1"
        width="640"
        height="360"
        controls
      >
