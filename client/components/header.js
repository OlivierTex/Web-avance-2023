      ...
      <header className="header">
        <div className="flex justify-between items-center">
          ...
          <div className="flex justify-end space-x-2">
            ...
            <Link href={isAuthenticated ? "/account" : "/account/login"} className="bg-gray-300 text-white px-4 py-2 rounded-md">
              <img className="w-8 h-8" src="/images/user-check-solid.svg" alt="Compte" style={{ display: isAuthenticated ? 'block' : 'none' }} />
              <img className="w-8 h-8" src="/images/user-solid.svg" alt="Compte" style={{ display: isAuthenticated ? 'none' : 'block' }} />
            </Link>
            {isAdmin && (
              <Link href="/account/admin" className="bg-gray-300 text-white px-4 py-2 rounded-md">
                <img className="w-8 h-8" src="/images/user-secret-solid.svg" alt="Admin" />
              </Link>
            )}
          </div>
        </div>
